/*  =========================================================================
    param_poller_selftest.c - run selftests
...
{
    test_item_t *item;
    printf ("Running param_poller selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef PARAM_POLLER_BUILD_DRAFT_API // selftest is still in draft
        else
            param_poller_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("param_poller_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running param_poller test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef PARAM_POLLER_BUILD_DRAFT_API // selftest is still in draft
        else
            param_poller_private_selftest (verbose, test->subtest);
#endif // PARAM_POLLER_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
